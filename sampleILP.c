...
// LINEAR PROGRAM
//
// min a'x, s.t. x>=0, b'x<=c, c>=0, x's whole numbers, where a, b, c and its dimension D are problem parameters
...
void initial(void *newsolution) {
	...
	for(j=0;j<D;j++) {
		((tsolution *)(newsolution))->x[j] = 0;  // Note it fulfills constraint b'x >= 0
	}
}
...
void show(void *solution, FILE *F) {
	...
	fprintf(F,"x=[");
	for(j=0;j<D;j++) {
		fprintf(F,"%d, ", ((tsolution *)(solution))->x[j]);
	}
	fprintf(F,"]\n");
}
...
void copy(void *currentsolution, void *newsolution) {
	memcpy( ((tsolution *)newsolution)->x, ((tsolution *)currentsolution)->x, D*sizeof(int));
}
